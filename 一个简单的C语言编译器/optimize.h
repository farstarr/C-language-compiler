...
void Generation(int start, int end) {//生成四元式
	for (int i = 0; i < Tree.size(); i++){
      ...
      && Tree[i].w != "value" && Tree[i].w != "param" && Tree[i].w != "END"&& Tree[i].w != "TEM"&& !(Tree[i].w == "!" && Tree[i].q3 == " ")) {//叶节点
      ...
      && Tree[i].w != "value" && Tree[i].w != "param" && Tree[i].w != "END" && Tree[i].w != "TEM" &&!(Tree[i].w == "!" && Tree[i].q3 == " ")){//非叶节点
